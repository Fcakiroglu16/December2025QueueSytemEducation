...
{
    public class MyStreamConsumer(RedisService redisService, ILogger<MyStreamConsumer> logger) : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                foreach (var entry in entries)
                {
                    try
                    {
                        logger.LogInformation("1.Consumer => Processing message {MessageId}", entry.Id);
